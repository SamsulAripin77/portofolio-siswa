<x-app-layout>
            ...
            <tbody class="bg-gray-50">
                ...
                <tr class="py-3 text-left">
                    ...
                    <td class="px-2 py-3 flex gap-2">
                        @if (Auth::user()->isAdmin())
                        <a href="{{route('sertifications.validation', [$item,$item->validation])}}">
                        ...
                        @endif
                        <form action="{{route('sertifications.destroy', $item)}}" method="post"
    ...
    <x-modal.create>
        ...
        <form action="{{route('sertifications.store')}}" method="post" enctype="multipart/form-data">
            @csrf
...
<script>
    function editMe(e,item){
        e.preventDefault()
        console.log(item)
        document.querySelector('#modal-edit form').action = 'sertifications/' + item.id
